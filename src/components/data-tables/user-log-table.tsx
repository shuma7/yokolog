...
export function UserLogTable({
  ...
  const cellIconSize = isMinimal ? 'h-3 w-3' : (isMinimal === false ? 'h-3 w-3' : 'h-4 w-4'); 
  const actionIconSize = isMinimal ? 'h-3 w-3' : (isMinimal === false ? 'h-3 w-3' : 'h-4 w-4'); 
  ...
  const actionButtonSizeClasses = isMinimal
    ...
    : (isMinimal === false ? "p-0.5 h-5 w-5 flex items-center justify-center" : "p-1 h-7 w-7"); 
